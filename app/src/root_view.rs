    ...
    fn handle_agent_onboarding_event(
        ...
        match event {
            ...
            AgentOnboardingEvent::OnboardingCompleted(selected_settings) => {
                ...
                // warpinator: AI runs against the local warpinator bridge (bring-your-own-key),
                // so completing onboarding with AI enabled never requires a Warp account.
                // Upstream forced login here whenever AI / Warp Drive was enabled and the user
                // wasn't signed in; we drop that requirement so the open-source build is usable
                // with no account.
